...
[![c2pa-rs](https://img.shields.io/badge/c2pa--rs-0.90.11-informational)](https://github.com/contentauth/c2pa-rs/releases/tag/c2pa-c-ffi-v0.90.11)
...
### 2a. Update Schemas and Regenerate Standard Assertions
...
4. Regenerate `lib/StandardAssertions.cs` from the updated standard-assertions schema:

```bash
./scripts/generate-assertion-data.sh
```

Optionally pin quicktype when regenerating:

```bash
QUICKTYPE_VERSION=23.2.6 ./scripts/generate-assertion-data.sh
```
...
6. Commit updated artifacts together (typically submodule pointer, `schemas/c2pa-standard-assertions.schema.json`, and `lib/StandardAssertions.cs`).

Note: `lib/Bindings/*.cs` is generated by ClangSharp and should not be edited manually.
